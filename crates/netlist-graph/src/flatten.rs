...
//! a [`FlattenedView`] with full incidence including the ground node.
//!
...
//! [`netlist_graph`] context to the rest of the solver pipeline.
//!
//! # Design references
//!
...
//!   That is the contract this module honors. Per ADR-0003 the
//!   [`FlattenedView`] type alias is the contract name exported by the
//!   netlist-graph crate; the concrete type is
//!   [`FlattenedStructure`] in `circuit-solver-types`.
//! - **`design.md` C4 L1**: the Flattener box originally sat inside
//!   the `numeric-solver` crate, but ADR-0003's contract table
//!   explicitly lists `netlist.FlattenedView` as owned by the netlist
//!   context. This module relocates the flattening walk into
//!   `netlist-graph`, where the `CircuitGraph` it consumes also lives.
//!   The `numeric-solver` crate re-exports [`flatten`] and
//!   [`FlattenError`] from here for backward compatibility.
...
//!      contract in [`CircuitGraph`], `build()`
...
use crate::{CircuitGraph, ElementKind};

/// The per-analysis flattened-view contract (ADR-0003).
///
/// `FlattenedView` is the contract name specified in ADR-0003's
/// "Shared Contracts" table: it is owned by the netlist context and
/// produced by [`flatten`]. The concrete type is
/// [`FlattenedStructure`] from `circuit-solver-types`, promoted there
/// to break the netlist-graph ↔ numeric-solver dependency cycle.
///
/// Consumers should use `FlattenedView` when they want to name the
/// contract without depending on the concrete type's definition crate
/// directly; the alias is re-exported from the netlist-graph crate
/// surface.
pub type FlattenedView = FlattenedStructure;
...
/// produce a [`FlattenedView`] with full incidence including the
/// ground node.
///
...
pub fn flatten(graph: &CircuitGraph) -> Result<FlattenedView, FlattenError> {
    let node_count: u32 =
...
mod tests {
    use super::*;
    use crate::{CircuitBuilder, GROUND_NET};
    ...
    // ---------- FlattenedView is FlattenedStructure -----------------------

    #[test]
    fn flattened_view_is_flattened_structure() {
        // The ADR-0003 contract name `FlattenedView` is a type alias
        // for `FlattenedStructure`. This test asserts the alias holds
        // so a future refactor does not silently break the contract.
        let mut b = CircuitBuilder::default();
        add_resistor(&mut b, "R1", "n1", GROUND_NET, 100.0);
        let g = b.build().expect("ok");

        let view: FlattenedView = flatten(&g).expect("flatten ok");
        let structure: &FlattenedStructure = &view;
