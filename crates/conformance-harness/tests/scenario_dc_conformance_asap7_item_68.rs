...
//! — **ASAP7 PDK variant**.
//!
//! This file is the executable witness for tasks.md item **#68** —
//! *"Implement ASAP7 PDK conformance test variant for DC and
//! transient"* — transposing the item #63 Sky130 harness scaffolding
//! onto ASAP7-relevant parameter values.
//!
...
//! ```
//!
//! *Note:* The spec scenario names "Sky130" — this witness is the
//! **ASAP7 transposition** per tasks.md item #68. The scenario shape
//! (conformance tester, golden reference, tolerance envelope, pass/fail)
//! is identical; only the PDK identity and parameter values change.
//!
...
//!
//! # What "ASAP7 PDK test bench" means here
//!
//! The ASAP7 PDK is a predictive 7 nm `FinFET` process. Its analog
//! primitives use BSIM-CMG — a device model **out of scope for v1**
//! per ADR-0005's closed-enum dispatch (MOSFET Level-1 only).
//! The "ASAP7 test bench" framing therefore refers to the
//! *test-bench shape* — a single NMOS biased through a series
//! resistor from a 0.7 V supply (the ASAP7 core voltage) — not to a
//! BSIM-CMG-fidelity comparison.
//!
//! The Level-1 parameter values used here are chosen to land in the
//! same drain-current order-of-magnitude as a comparable 7 nm
//! low-VT NFET bias leg (~tens of µA), so the **envelope arithmetic**
//! is exercised at realistic ASAP7 magnitudes (`1 % relative` of
//! `0.7 V` is `7 mV` of slack; `1 mV absolute` floor protects
//! near-zero ground nodes from numerical noise).
//!
//! This is the same scaffolding-transposition pattern documented in
//! the Sky130 sibling's module docstring (line 61–63): "only the
//! parameter set and golden values change."
//!
//! # Golden reference choice
//!
//! Per the glossary, *"a trusted external simulator against which
//! results are compared."* The canonical such simulator for ASAP7
//! would be `ngspice`. ngspice is **not available** at test-runtime
//! in this sandbox, so this witness uses a **synthesized ngspice
//! rawfile** whose values are derived from the *same Level-1
...
// --- ASAP7-flavored fixture parameters ---------------------------------------

/// ASAP7 core supply voltage. The 7 nm predictive PDK's nominal
/// digital core rail is 0.7 V; this matches that.
const VDD_VOLTS: f64 = 0.7;

/// Series resistor between the supply and the diode-connected NMOS
/// drain/gate node. 1 kΩ is a representative on-chip poly resistor
/// at 7 nm; it sets the operating current in the tens-of-µA decade
/// typical of a low-VT NFET reference leg.
const R_SERIES_OHMS: f64 = 1_000.0;

/// Threshold voltage `VTO` for the Level-1 NMOS, in volts. 0.26 V
/// approximates the low-VT flavor on the ASAP7 predictive 7 nm
/// `FinFET` process (the PDK's RVT devices sit around 0.25–0.30 V).
const NMOS_VTO_V: f64 = 0.26;

/// Level-1 transconductance parameter `KP = µ·Cox·(W/L)`, in A/V².
/// 500 µA/V² reflects the higher drive current density of a 7 nm
/// `FinFET` compared to the 180 nm planar Sky130 IO device; with the
/// chosen `R_SERIES_OHMS` it gives a converged drain current
/// `≈ 40 µA`, well inside the typical ASAP7 low-VT NFET DC bias
/// envelope.
const NMOS_KP: f64 = 500e-6;
...
/// `DeviceModelBinding`. Naming mirrors an ASAP7 SPICE deck's typical
/// `XM1 ... asap7_nfet_rvt` style; the exact string is only used for
/// the `ModelName` lookup inside the orchestrator.
const NMOS_MODEL_NAME: &str = "asap7_nfet_rvt_level1";
...
/// Synthesize an ngspice ASCII rawfile body for the "ASAP7 low-VT NFET
...
fn synthesize_golden_rawfile() -> String {
    ...
    //   v(n_vdd) = 0.7 V exactly (pinned by V1).
    //   v(n_dg)  = Vto + u    (closed-form above).
    format!(
        "Title: asap7-rvt-nfet-reference-leg-op\n\
...
fn write_temp_fixture(name: &str, body: &str) -> std::path::PathBuf {
    let dir = std::env::temp_dir().join("conformance-harness-dc-item-68");
    std::fs::create_dir_all(&dir).expect("create dc-item-68 temp dir");
...
/// Build the ASAP7-flavored "low-VT NFET reference leg" test bench:
///
/// ```text
///   V1 (0.7 V, n_vdd → 0) ──── R1 (1 kΩ) ──── n_dg ──── M1.drain
///                                                         │   .gate ── tied to n_dg
///                                                         └── .source = .bulk = 0
/// ```
///
...
/// resolution by name).
fn asap7_rvt_nfet_reference_leg() -> (FlattenedStructure, CircuitGraph) {
...
fn asap7_nfet_bindings() -> Vec<DeviceModelBinding> {
...
/// pair shape the conformance comparator expects.
...
/// **Headline scenario witness (ASAP7 variant).**
///
/// > Given ConformanceTester has a ngspice Golden Reference for an
/// > ASAP7 PDK test bench
/// > And the tolerance envelope is configured as 1 % relative or
/// > 1 mV absolute per node
/// > When ConformanceTester runs the DC operating-point Analysis on
/// > the same Circuit
...
/// > And Conformance is reported as "pass"
#[test]
fn dc_conformance_against_asap7_rvt_nfet_golden_reports_pass() {
    // Given: ConformanceTester has a ngspice Golden Reference for an
    // ASAP7 PDK test bench. (The synthesized rawfile encodes the
    // closed-form Level-1 operating point; see module docstring for
    // why an in-process synthesis stands in for the real `ngspice
    // -b asap7_rvt_nfet.spi -r golden.raw` invocation.)
    let golden_path = write_temp_fixture(
        "dc-conformance-asap7-pass.raw",
    ...
    // for is the ASAP7 RVT NFET reference-leg fixture below.
    let (fs, g) = asap7_rvt_nfet_reference_leg();
    let bindings = asap7_nfet_bindings();
...
/// **Negative-path companion (ASAP7 variant).**
///
/// A perturbed golden — same shape, same variables, but with `v(n_dg)`
/// offset by 40 mV (well outside the `max(1 % · 0.7 V, 1 mV) = 7 mV`
...
fn dc_conformance_fails_when_golden_is_perturbed_beyond_envelope() {
    // Synthesize a perturbed golden: `v(n_dg)` shifted by 40 mV.
    // The closed-form `Vgs` from `analytic_operating_point()` is the
    // *correct* value the solver will hit, so the perturbation lives
    // entirely in the golden file. The 40 mV offset is chosen so
    // that even at the largest legitimate Vgs the envelope cannot
    // close: `max(0.01 · |Vgs_ref|, 1e-3) ≤ 0.01 · 0.7 V = 7 mV` —
    // 40 mV is unambiguously outside.
    let (v_dg_correct, _id) = analytic_operating_point();
    let v_dg_perturbed = v_dg_correct + 0.040; // +40 mV

    let body = format!(
        "Title: asap7-rvt-nfet-reference-leg-op-PERTURBED\n\
         Plotname: Operating Point\n\
         Flags: real\n\
         No. Variables: 3\n\
         No. Points: 1\n\
    ...
    let golden_path = write_temp_fixture("dc-conformance-asap7-fail.raw", &body);
    ...
    // v(n_vdd) is pinned at 0.7 V exactly by V1 — it must still pass.
    ...
    // v(n_dg) carries the single failure with the 40 mV-ish offset.
    ...
    // precision, not by `f64` round-trip.
    assert!(
    ...
    // Margin is `envelope − |diff|`. Diff ≈ 40 mV; envelope at this
    // Vgs is ~ max(0.01·|0.66|, 1e-3) ≈ 6.6 mV → margin ≈ −33 mV.
    assert!(
        fail.margin < -0.005,
...
/// **Sanity pin (ASAP7 variant)** — the closed-form analytic operating
/// point and the solver-produced operating point agree to within the
/// same envelope the conformance test uses. This is a tighter
/// constraint than the `Pass` verdict alone (it asserts the *solver*
/// converges to the right answer, not just that the solver's answer
/// matches its own echo through the rawfile). Without this pin a
/// regression that broke both the solver *and* the golden synthesis
/// identically would still slip through.
#[test]
