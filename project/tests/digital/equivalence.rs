...
fn multi_net_trace_with_tolerance() {
    // Each net's events shift independently but stay within tolerance.
    // Events on different nets at the same timestamp retain their sort order
    // in both traces, so positional comparison works correctly.
    let actual = make_trace(&[
        (0.0, "clk", LogicValue::One),
    ...
    // 2ps tolerance should cover both deviations
...
fn first_mismatch_is_reported() {
    ...
    let result = check_equivalence(&actual, &expected, &EquivalenceTolerance::exact());
    assert!(!result.equivalent);
    // Should report index 0, not index 1
    assert!(matches!(
        result.first_mismatch,
        Some(EquivalenceMismatch::ValueMismatch { index: 0, .. })
    ));
...
fn zero_tolerance_requires_exact_times() {
    let actual = make_trace(&[(1.000_000_000_000_001, "a", LogicValue::One)]);
    let expected = make_trace(&[(1.0, "a", LogicValue::One)]);
    let _result = check_equivalence(&actual, &expected, &EquivalenceTolerance::exact());
    // Whether this fails depends on f64 precision; the delta here is ~1e-15
    // which is within one ULP of 1.0, so (1.0 - 1.0).abs() == 0.0 in f64
    // if the value rounds to exactly 1.0.  Let's use a clearly different value:
...
fn push_maintains_sort_with_interleaved_nets() {
    ...
    // Push events in reverse-time order across multiple nets
    for i in (0..50).rev() {
        let t = i as f64 * 1e-9;
