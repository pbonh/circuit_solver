...
2. **Reproduce the conflict.** `cd $HERMES_KANBAN_WORKSPACE`;
   `git checkout gamma` (this board targets `gamma` — never `beta` or `main`);
   record `gamma`'s SHA; attempt the merge (`git merge <branch>`).
...
| Contract | Owner | Ratified-by | Shape |
|----------|-------|-------------|-------|
| `netlist.CircuitGraph` | netlist | ADR-0001 | Immutable graph built by frontend, consumed by orch |
...
| `devices.DeviceModel` | devices | ADR-0005 | Closed enum; static monomorphized dispatch |
...
CircuitGraph. Binding crate depends only on `circuit-solver-frontend`.

**ADR-0002 (inherited, effective 1.045):** Pure-Rust hybrid sparse-direct solver:
russell (real DC/transient) + faer (complex AC). No C/C++ FFI.

**ADR-0003 (inherited, effective 1.045):** Two-pass graph flattening with
per-analysis sub-views. `FlattenedView` shared between `orchestration` and `numeric`.
...
Supersedes ADR-0004. In-process `run-until` only; no cross-process IPC.
Icarus Verilog is the golden reference for validation, not the runtime engine.
...
Refines ADR-0005; runtime registration remains rejected.

**ADR-0008 (accepted, effective 0.95):** Cargo workspace layout; one crate per
bounded-context container; compiler-enforced cross-crate boundaries.
